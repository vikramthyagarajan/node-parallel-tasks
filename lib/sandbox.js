...
Sandbox.prototype.initializeEnvironment = function(data,finalCallback) {
	map=data.mapFunction;reduce=data.reduceFunction;array=data.array;scope=data.scope;
	if(map&&typeof map=='string'){
		this.mapFunction=eval('('+map+')');
	}
	else this.mapFunction=map;
...
Sandbox.prototype.initializeParallelEnvironment = function (data, finalCallback) {
	map=data.mapFunction;reduce=data.reduceFunction;scope=data.scope;
	if(map&&typeof map=='string'){
		this.mapFunction=eval('('+map+')');
	}
	else this.mapFunction=map;
...
Sandbox.prototype.executeParallelMapFunction = function(finalCallback) {
	this.mapFunction.call(this.scope);
	finalCallback();
};
/**
...
Sandbox.prototype.executeParallelReduceFunction = function(err, results, finalCallback) {
	this.reduceFunction.call(this.scope);
	finalCallback();
};
