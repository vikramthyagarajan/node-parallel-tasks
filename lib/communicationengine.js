...
var clientEvents=['requestResource','readyEnvironment','readyParallelEnvironment','startExecution','startParallelExecution','finish'];
var serverEvents=['resourceAvailable','environmentReady','parallelEnvironmentReady','executionDone','parallelExecutionDone'];
...
CommunicationEngine.prototype.initializeEngine = function() {
	...
	this.socket.on('readyParallelEnvironment',function(data){
		winston.info('asked to ready environment');
		commEngine.executionEngine.initializeParallelEnvironment(data,function() {
			commEngine.status='environmentInitialized';
			commEngine.socket.emit('parallelEnvironmentReady');
		});
	});
	...
	this.socket.on('startParallelExecution',function(data){
		winston.info('starting execution');
		commEngine.executionEngine.executeParallelMapFunction(function(err,results) {
			commEngine.status='processed';
			commEngine.socket.emit('parallelExecutionDone',{err:err,results:results});
		});
	});
	...
	this.socket.on('parallelEnvironmentReady',function(data){
		winston.info('a resource has initialized its environment');
		commEngine.status='environmentInitialized';
		commEngine.executionEngine.onSocketParallelEnvironmentInitialized(commEngine);
	});
	...
	this.socket.on('parallelExecutionDone',function(data){
		commEngine.executionEngine.parallelReduce(commEngine,data.err,data.results,function() {
			commEngine.status='idle';
			commEngine.socket.emit('finish');
		});
	});
...
CommunicationEngine.prototype.emitParallelEnvironmentData = function(data) {
	this.socket.emit('readyParallelEnvironment',data);
};
