...
/** Emit Parallel Data */
var onParallelTimeout = function(resources,sandbox,socketCommunicator,currentEngine) {
	if(resources.length!=0){
		var commonData=sandbox.getSandboxEmitData();
		resources.forEach(function(resource){
			var data = _.clone(commonData);
			resource.emitParallelEnvironmentData(data);
		});
	}
	else{
		ExecutionEngine.executeParallelLocally(sandbox, currentEngine);
	}
}
/** Emit Parallel Data */
...
/** Parallel function  */
ExecutionEngine.prototype.executeParallel = function(map, reduce, options, callback) {
	if (!options) {
		options = {
			scope: {}
		};
	...
	if(this.status=='idle'){
		...
		var that=this;
		setTimeout(function() {
			onParallelTimeout(that.resources,that.sandbox,that.socketCommunicator,that);
		},config.resourceAcquisitionTimeout);
	}
};
...
ExecutionEngine.prototype.initializeParallelEnvironment = function(data,finalCallback) {
	this.sandbox.initializeParallelEnvironment(data,finalCallback);
};
/**
...
ExecutionEngine.prototype.executeParallelMapFunction = function(finalCallback) {
	this.sandbox.executeParallelMapFunction(finalCallback);
};
/**
...
ExecutionEngine.prototype.onSocketParallelEnvironmentInitialized = function(socketCommEngine) {
	//check if everyone has initialized their env
	winston.info('one person has initialized environment');
	var ind=_.findIndex(this.resourcesEnvironmentInitialized,{id:socketCommEngine.socket.id});
	if(ind==-1){
	...
	if(this.resources.length==this.resourcesEnvironmentInitialized.length){
		winston.info('everyone has initialized their environment');
		this.socketCommunicator.emitEvent('startParallelExecution');
	}
};
/**
...
ExecutionEngine.prototype.parallelReduce = function(socketCommEngine,err,results,finalCallback) {
	winston.info('one person has executed map');
		var that = this;
		this.sandbox.executeParallelReduceFunction(this.err,this.results, function(err, results) {
			finalCallback(err, results);
			var oldCallback = that.currentCallback;
			winston.info('resetting the callback. MapReduce is done');
			that.currentCallback = that.defaultCallback;
...
/** Execute Parallel Locally */
ExecutionEngine.executeParallelLocally = function(sandbox, execEngine) {
	async.map(sandbox.mapFunction,function(err,results) {
		sandbox.reduceFunction(err,results);
		execEngine.status='idle';
	});
};
